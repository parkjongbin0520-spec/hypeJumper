...
**비주얼 기초(6-1~6-5) 전부 완료**

---

### 카메라 재구성 (2026-06-02) — 셀레스트식 방 고정 + 줌 3x

- [x] camera.py 재작성 — 자동 그리드 방(320×180) 고정, 경계 클램프, 방 전환 빠른 슬라이드
- [x] scene.py — 슬라이드 중 게임플레이 정지(셀레스트식), 로드/리스폰 `snap_to`
- [x] main.py — 내부 320×180 렌더 → 960×540 3배 확대, HUD 네이티브
- [x] settings — CAMERA_ZOOM=3 / INTERNAL_W/H / ROOM_SLIDE_LERP / **player·물리 무수정**
- [x] QA 11/11(방전환·슬라이드·정지·줌·레벨전환 회귀·240f 예외0) + WORK_LOG (19)

**검수**: `python main.py` — 2.5x 확대, 방 단위 고정, 경계 넘으면 옆 방 슬라이드.
**튜닝 가능**: 줌 배율(CAMERA_ZOOM=2.5) / 슬라이드 속도(ROOM_SLIDE_LERP).
**[피드백 반영]** 줌 3→2.5x / 방 전환 시 대시 초기화(MAX 충전)
**[버그수정]** 지상 점프 시 방 갈림 → 히스테리시스(보이는 방을 완전히 벗어날 때만 전환). 매프레임 격자 재계산 폐기.

---

### level1 방 단위 재구성 (2026-06-02) — 3×2 메카닉 쇼케이스

- [x] level1.txt 재생성: 1152×432 = 가로3×세로2 방(각 384×216)
  - 하단: R1 스폰 → R2 대시갭+체크포인트 → R3 등반사다리
  - 상단: R4 이동발판+착지 → R5 잡기NTT/적 → R6 골 (우측 등반으로 하단↔상단 연결)
- [x] 로드 검증(3×2방·스폰·골·객체) + map_overview_level1.png 육안
- [x] WORK_LOG (22)

**검수**: `python main.py` — 방별 메카닉·우이동/상향등반 방전환·대시갭·골 도달.
**튜닝**: 등반 사다리 간격/발판 타이밍/난이도 손맛 따라 조정. level2는 아직 미정렬(추후).
**다음**: 맵 손맛 조정 / level2 재구성 / UI·투사체 적
