...
- **수정 파일**: settings.py (INTERNAL_W/H 정의)
- **요청(비니)**: 세션 재개 — "줌/방 정렬 먼저". (맵 작업 중 중단된 상태에서 정렬부터.)
- **증상/원인**: 줌2 → 내부 렌더(=한 방) 480×270. 가로 480=30타일 정확하나, **세로 270이 타일16에 안 나눠떨어짐(16.875타일)** → 방 격자선(y=270,540,810)이 타일 경계를 가로질러 세로 방전환이 ~2px 어긋남. 정작 비니가 만든 맵들은 이미 **17타일(272px) 방** 기준: level2~5=480×272, level1=480×816(=272×3). 즉 맵은 272 정렬인데 settings의 INTERNAL_H만 270으로 남아 불일치.
- **수정**: 방 크기를 화면÷줌이 아니라 **타일 정수배로 정의**. `ROOM_W_TILES=30`/`ROOM_H_TILES=17` 신설 → `INTERNAL_W=480`(30타일, 불변) / `INTERNAL_H=272`(17타일, 270→272). 내부 surface 480×272는 main의 `transform.scale`이 화면 960×540으로 확대(가로 ×2.0 / 세로 ×1.985 = 0.74% 차, 육안 무시). CAMERA_ZOOM=2 유지(주석만 보정).
- **영향 범위**: INTERNAL_W/H 사용처 = camera.py(뷰 기본값)·main.py(world surface) 2곳뿐, 둘 다 값만 읽음. player·물리·엔티티·맵데이터 전부 무수정.
- **검증(자동)**: 5개 맵 전부 rooms 정수 정렬 — level1=1×3 / level2~5=1×1 **OK**(이전 1.01·3.02 어긋남 해소). Game 헤드리스 600프레임(우이동+점프+대시+상향) world 480×272·방 row0→2 슬라이드·예외0 **SMOKE OK**.
- **원본 파일 업데이트 여부**: 없음. (설정 상수 정렬 수정. ROOM_W/H_TILES 신설은 settings 내 추가.)
- **참고**: WORK_LOG (22)는 옛 level1(1152×432, 3×2방, 줌2.5) 서술 — 현 구조(줌2, 세로 1방폭·17타일 방, level1=480×816 세로3방)와 다름. (22) 원문은 규칙#8로 미수정, 현행 구조는 본 (23) 기준.
- **다음 작업**: 비니 직접 확인 — `python main.py` 세로 방전환이 타일 경계에 딱 맞는지. 이후 빈 스텁(level2~5) 콘텐츠 채우기 또는 커밋/기록 정리 지시 대기.

## 2026-06-02 (22)

### [Phase 4] level1 재구성 — 방 단위(3×2) 메카닉 쇼케이스 맵

- **수정 파일**: assets/tilemaps/level1.txt (재생성)
- **요청(비니)**: 방 분리했으니 맵을 방 단위로 재구성. 성격=메카닉 쇼케이스, 규모=3×2 그리드.
- **설계**: 맵 1152×432 = 가로3방×세로2방(각 384×216px=24×13.5타일, 세로 2방=432=27타일로 정수 정렬). 방 경계는 카메라 분할일 뿐 지형은 연속.
  - 하단(우 이동): R1 스폰/기본이동·점프 → R2 대시 갭(바닥 구멍, 낙사) + 체크포인트 → R3 우측 등반 사다리(점프 상승)
  - 우측 사다리로 상향 방전환 → 상단(좌 이동): R4 이동발판(상단 바닥 갭 건넘)+착지 → R5 잡기 NTT+적 → R6 골
  - 경로: 스폰→우→대시→우→등반(↑방전환)→발판→좌→잡기/적→골(루프형)
- **객체**: 이동발판 1(상단 갭 50~54 건넘), 적 1(R5), NTT 1(R5 공중), 체크포인트 1(대시갭 뒤), 골 1(R6). 대시는 방 진입 시 자동충전이라 갭/발판 안전망.
- **검증(자동)**: 로드 — 1152×432=3×2방 / solids58 / 스폰(48,384) R1 / 골(64,176) R6 / cp·plat·enemy·ntt 각 배치 확인. map_overview_level1.png(방 격자선 포함) 육안 — 방별 콘텐츠 정렬·지형 연속·등반 연결·대시갭 확인.
- **원본 파일 업데이트 여부**: 없음. (맵 데이터 재생성. level2.txt는 유지 — 추후 방 정렬 가능.)
- **다음 작업**: 비니 직접 플레이 — 방별 메카닉·방 전환(우 이동/상향 등반)·대시갭·골→level2 확인. 손맛 따라 방 배치/난이도 조정. 커밋 대기.

## 2026-06-02 (21)

### [Phase 4][버그수정] 지상 점프 시 방이 갈리는 문제 — 히스테리시스(영역 이탈 시에만 전환)

- **수정 파일**: src/camera.py
- **버그(비니)**: 2.5x 전환 후, 지상에서 점프하면 방이 갑자기 나뉨(엄청 불편).
- **원인**: `update`가 **매 프레임 플레이어 중심//뷰크기로 방 격자를 재계산** → 점프로 중심 y가 격자선(예 row 경계 648)을 넘는 순간 위 방으로 전환. 줌 2.5라 방 높이 216px, 격자선이 바닥 바로 위(~160px 이내가 아니라 클램프 무시한 격자선)에 걸려 점프(정점 ~50px)만으로도 경계 교차. (클램프된 표시영역이 아니라 격자셀 기준으로 판정한 게 화근)
- **수정**: 셀레스트식 **히스테리시스** — 현재 '보이는 방(클램프된 표시 rect)'을 **완전히 벗어났을 때만** 이웃 방으로 전환. `update`에서 매 프레임 격자 재계산을 멈추고, `dox<=cx<dox+vw and doy<=cy<doy+vh`(현재 표시영역 내부) 검사 → 벗어날 때만 `col,row` 갱신·슬라이드. `snap_to`는 중심 기준 방 인덱스 설정.
- **검증(자동)**: 바닥 방(표시 [504,720)) 점프 → 정점 center_y=615(>504, 영역 내) → **전환 없음·방 인덱스 불변** / 오른쪽으로 뷰 밖 이동 → 전환 발생 / Game 240f 예외0 — **4/4 PASS**. 방-전환 대시충전(20)·줌 2.5도 유지.
- **원본 파일 업데이트 여부**: 없음. (카메라 방 판정 버그 수정.)
- **다음 작업**: 비니 직접 플레이 — 지상 점프해도 방 안 갈리는지, 화면 밖 나갈 때만 옆 방 슬라이드 확인. 커밋 대기.

## 2026-06-02 (20)

### [Phase 4][피드백] 줌 3x→2.5x 하향 + 방 전환 시 대시 초기화

- **수정 파일**: settings.py, src/scene.py
- **피드백(비니)**: 1) 너무 확대됨 → 2.5배로. 2) 방 이동 시 대시 초기화되면 좋겠음.
- **변경 내용**:
  - settings.py: `CAMERA_ZOOM 3→2.5`. `INTERNAL_W/H = int(SCREEN/2.5)` = **384×216**(2.5×=정확히 960×540). `//`→`int(/)`로 변경(2.5 float 대응).
  - scene.py: `update` 끝 `camera.update()`가 **슬라이드 시작(True) 반환 시** `player.dashes = MAX_DASHES`. 라이징 엣지(전환 시작 프레임)에만 1회 — 슬라이드 중 프레임은 top-guard라 미중복, 방 변화 없으면 미충전.
- **검증(자동)**: 줌 2.5·내부 384×216·뷰 384×216 / 방 col0→1 전환 시 대시 0→MAX 충전 / **공중+방전환없음 → 대시 0 유지**(방-충전이 지상 자동충전과 분리됨 확인) / world surface 384×216 / Game 180f 예외0 — **PASS**. (1차 '미충전' FAIL은 플레이어가 지상이라 기존 지상 자동충전이 작동한 테스트 셋업 artifact → 공중 재검증 통과.)
- **원본 파일 업데이트 여부**: 없음. (줌 값 튜닝 + 방전환 대시충전은 카메라 재구성 보강.)
- **다음 작업**: 비니 직접 플레이 — 2.5배 확대 체감·방 이동 시 대시 회복. 커밋 대기.

## 2026-06-02 (19)

### [Phase 4] 카메라 재구성 — 셀레스트식 방 고정 + 빠른 슬라이드 + 줌 3x

- **수정 파일**: settings.py, src/camera.py(재작성), src/scene.py, main.py
- **승인**: 비니 지정(자동그리드 방분할 / 줌 3x / 빠른 슬라이드) + 구조 평가 보고서 승인("승인 — 이대로").
- **변경 내용**:
  - settings.py: `CAMERA_ZOOM=3`, `INTERNAL_W/H=320/180`(=한 방 크기), `ROOM_SLIDE_LERP=0.18`. (구 CAMERA_LERP는 미사용 주석)
  - camera.py **재작성**: 추적 lerp 폐기 → **방 고정**. `_room_origin`(플레이어 중심//뷰크기로 격자칸 산출, 맵 경계 클램프), `snap_to`(즉시 방 고정), `update`(방 인덱스 바뀌면 슬라이드 시작, 진행 중이면 목표로 보간, 같은 위치면 슬라이드 생략, 슬라이드 중 True).
  - scene.py: `update` 맨앞 — `camera.sliding`이면 게임플레이 전체 스킵하고 카메라만 이동(셀레스트식 전환 정지). `_load_level`·`respawn`은 `snap_to`로 방 스냅.
  - main.py: 내부 `self.world`(320×180)에 씬 렌더 → `transform.scale`로 화면 960×540 **3배 확대**. HUD는 확대 후 네이티브에 그려 선명.
  - **player.py·엔티티·물리 전부 무수정**(draw는 offset 그대로, 더 작은 surface에 그릴 뿐 / 전환 정지는 Scene이 update 스킵).
- **검증(자동)**: 뷰=320×180 / 스폰 방 원점 클램프(0,540) / 방 col 0→1 전환 시 슬라이드 시작 / **슬라이드 중 게임플레이 정지(입력 줘도 player.x 불변)** / 슬라이드 완료 후 방1 원점(x=320) 스냅 / world surface 320×180·화면 960×540 / 레벨 전환(골→snap_to) 회귀 / Game 240f 예외0 — **11/11 + 회귀 PASS**. map_preview_zoom.png 3x·방고정 육안 확인.
- **원본 파일 업데이트 여부**: 없음. (카메라 룩/방식은 CLAUDE Phase4 범주. 방고정·줌은 비니 지정 구현.)
- **다음 작업**: 비니 직접 플레이 — `python main.py` 3x 확대·방 단위 고정·경계 넘으면 옆 방 슬라이드 손맛. 승인 시 슬라이드 속도/줌 튜닝 또는 다음. 커밋 대기.
